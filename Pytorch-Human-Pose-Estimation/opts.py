...
class opts():
	...
	def init(self):
		...
		#linux可以使用多线程 而windows不能
		#self.parser.add('--num_workers', default=0, type=int , help='No. of parallel threads')
...
######## DataLoader Parameters
	...
	def parse(self):
		...
		self.opt.saveDir = os.path.join(os.path.join(self.opt.expDir, self.opt.expID), os.path.join(self.opt.model, 'logs_{}'.format(datetime.datetime.now().isoformat().replace(' ', '_').replace(':', '.'))))
		self.opt.saveDir = os.path.join(self.opt.expDir, self.opt.model, self.opt.expID, 'logs_{}'.format(datetime.datetime.now().isoformat().replace(' ', '_').replace(':', '.')))
